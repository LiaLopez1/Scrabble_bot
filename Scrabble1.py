...
class ScrabbleGUI:
    ...
    def actualizar_display_atril(self):
        for label, ficha in zip(self.labels_atril, self.atril):
            texto = '' if ficha is None else ficha.upper()  # Mostrar en blanco para el comodín
    ...
    def crear_boton_confirmar(self):
        self.boton_confirmar = tk.Button(self.master, text="Confirmar palabra", font=('Arial', 18), command=self.confirmar_palabra)
        self.boton_confirmar.pack(side=tk.TOP, pady=20)

    def confirmar_palabra(self):
        # Aquí puedes implementar la lógica adicional que necesites para validar la palabra, actualizar puntajes, etc.
        self.rellenar_atril()
        self.fichas_colocadas = []
